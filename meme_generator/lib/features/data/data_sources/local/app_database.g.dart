    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `template` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `uriToImage` TEXT, `textList` TEXT NOT NULL)');
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `textdata` (`id` INTEGER, `text` TEXT NOT NULL, `position` REAL NOT NULL, PRIMARY KEY (`id`))');
...
class _$TemplateDao extends TemplateDao {
  ...
  Future<List<TemplateModel>> getTemplates() async {
    return _queryAdapter.queryList('select * from template',
        mapper: (Map<String, Object?> row) => TemplateModel(row['id'] as int?,
